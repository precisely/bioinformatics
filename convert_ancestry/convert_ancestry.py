...
Output files are sent to: 'bioinformatics/convert_ancestry/test/<sample_fakehash#>.txt'
'''
...
def format_metadata(datetime_obj, genome_version):
    ...
    metadata = ''
    metadata += '# This data file generated by 23andMe at: {}\n'.format(datetime_obj.strftime('%c'))
...
def format_data(line):
    ...
    if line.startswith('rs'):
        ...
        if len(fields) < 5:
            raise ValueError('Error parsing data entry: {}'.format(str(fields[0])))
...
def convert_data_rows(input_stream, output_stream):
    ...
    for line in input_stream:
        output_stream.write(format_data(line))     
...
def parse_metadata(input_stream):
    ...
    while is_header_line(line):
        ...
        line = input_stream.readline()
        if line == None:
            raise StopIteration('Insufficient metadata')    
...
def convert_ancestry(input_stream, output_stream):
    ...
    output_stream.write(format_metadata(datetime_obj, genome_version))
